...
class ThomasLease(models.Model):
    ...
    rate_calc_description = fields.Char("Rate Calculation", compute='_compute_rate_calc_description')
    rate_calc_example = fields.Text("Rate Example", compute='_compute_rate_calc_example')
        ...
        for rec in self:
            for line in rec.lease_lines:
                if rec.rate_calc_description:
                    rec.rate_calc_description = str(rec.rate_calc_description) + str(line.product_id.description) + '\r\n'
                else:
        ...
        for rec in self:
            for line in rec.lease_lines:
                ...
                if line.product_id.rate_type == 'monthly' or line.product_id.rate_type == 'stelco_monthly':
                    example = '${0:,.2f}'.format(line.monthly_rate) + " per month\r\n" \
                              + '${0:,.2f}'.format(line.weekly_rate) + " per week\r\n" \
                              + '${0:,.2f}'.format(line.daily_rate) + " per day\r\n"
...
class ThomasFleetLeaseInvoiceWizard(models.TransientModel):
    ...
    def calc_rate_daily_lease(self, discount, daily_rate, start_date, end_date):
        ...
        cost_per_day_d_rate = daily_rate
        rate = daily_rate
        cost_per_day_w_rate = ((daily_rate / .125) * .45) / 7
        cost_per_day_m_rate = (((daily_rate / .125) * .45)/.45) / 30
        num_days = date_delta.days + 1  # assumes current day for billing
                    ...
                    if self.aggregate_lease_selected(lease):
                        ...
                        if lease.run_initial_invoicing:
                            for next_line in lease.lease_lines:
                                n_resp = self.calculate_line_amount(product,next_line, next_line.price, prev_month_from.strftime(
                                    '%Y-%m-%d'), prev_month_to.strftime('%Y-%m-%d'), lease)
